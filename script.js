...
async function foo(){
    var res=await fetch(`https://pokeapi.co/api/v2/pokemon/?offset=20&limit=1154`)
    ...
    function foo2(){
        results.slice(0, 50).forEach(ele=>{
            ...
            async function foo3(){
                ...
                var name=data1.name;
                var res1=await fetch(`https://pokeapi.co/api/v2/pokemon-form/${name}`)
                var data1s=await res1.json();
                var results1=data1s.sprites;
                const carousel=document.createElement("div");
                carousel.setAttribute("class","carrs col-sm-12 col-lg-3 col-md-4 mt-5 mb-4 d-flex justify-content-around")
...
button.addEventListener("click",async()=>{
    ...
    try{results.forEach(element => {
        if(element.name==lowercaseinput){
            const url=element.url;
            async function foo1(){
                var res1=await fetch(url);
                var data1 =await res1.json();
                var res1=await fetch(`https://pokeapi.co/api/v2/pokemon-form/${lowercaseinput}`)
                var data1s=await res1.json();
                var results1=data1s.sprites;
                const carousel=document.createElement("div");
                carousel.setAttribute("class","carrs col-sm-12 col-lg-3 col-md-4 mt-5 mb-4 d-flex justify-content-around")
